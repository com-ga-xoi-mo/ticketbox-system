...
export class UploadBannerUseCase {
  ...
  async execute(input: UploadPosterInput): Promise<UploadPosterResult> {
    ...
    const storageKey = `banners/${input.concertId}/${assetId}.${validated.extension}`;
    ...
    let result: Awaited<
      ReturnType<typeof this.posterWriteRepo.createAssetAndAssociateConcertBanner>
    >;
    try {
      result = await this.posterWriteRepo.createAssetAndAssociateConcertBanner(
        {
          id: assetId,
          storageKey,
          publicUrl,
    ...
    return {
      asset: result.asset,
      concert: { id: result.concert.id, posterAssetId: result.concert.bannerAssetId },
