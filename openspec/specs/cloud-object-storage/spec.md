...
Define the shared Object Storage foundation for TicketBox. It establishes a standard ObjectStoragePort interface and provides multiple adapters (S3-compatible for Cloudflare R2, local filesystem for development/testing, and in-memory for unit testing). This shared infrastructure ensures consistent object storage operations across the entire application while isolating downstream modules (such as ai-artist-bio) from the underlying storage provider details.
