...
- **Event search and filtering**: Add text search, city filter, date-range filter (`dateFrom`/`dateTo`), price-range filter (`minPrice`/`maxPrice`), sort controls (date, price), and empty/no-results states to the event listing page. Requires backend `GET /concerts` to accept query params (`q`, `city`, `dateFrom`, `dateTo`, `minPrice`, `maxPrice`, `sortBy`, `sortDir`) — propose a minimal catalog search API extension.
...
- `catalog-search-api`: Backend query parameter support on `GET /concerts` (`q`, `city`, `dateFrom`, `dateTo`, `minPrice`, `maxPrice`, `sortBy`, `sortDir`), new `GET /concerts/cities` endpoint, and shared Zod contract additions in `@ticketbox/api-types`.
...
- **Shared contracts (`packages/api-types`)**: New `CatalogSearchParamsSchema` (with date-range and price-range fields), `PublicConcertCitiesResponseSchema`, updated `PublicConcertListResponseSchema` imports.
