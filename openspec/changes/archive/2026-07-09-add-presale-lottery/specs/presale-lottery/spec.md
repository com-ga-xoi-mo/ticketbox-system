...
- **WHEN** a lottery is configured and its presale gate window is open but the draw has not run yet (no entitlements exist)
...
- **AND** each row SHALL include user display name, email, desired quantity, status, registered time, result timestamps, fulfillment timestamp, and linked entitlement status/expiry/order id when present
