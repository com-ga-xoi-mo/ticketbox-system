...
After a successful sign-in, the app SHALL route the user to a destination determined by their roles, applying a fixed precedence when a user holds more than one role: `ADMIN` outranks `ORGANIZER`. A user whose roles include neither `ADMIN` nor `ORGANIZER` (e.g. `CHECKIN_STAFF`, `AUDIENCE`) SHALL be routed to an access-denied destination rather than a portal page, to avoid a redirect loop.

#### Scenario: Organizer redirected to dashboard
...
- **THEN** the app navigates to the admin destination (`/dashboard` until the admin surface exists)
