...
export function MyTicketsPage() {
  ...
  const sortedTickets = tickets ? [...tickets].sort((a, b) => 
    new Date(a.concertStartsAt).getTime() - new Date(b.concertStartsAt).getTime()
  ) : [];
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">Quản lý vé điện tử và mã QR để check-in.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed">
            ...
            <p className="mb-6 text-muted-foreground">Bạn chưa có vé nào trong tài khoản.</p>
            <Button asChild>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sortedTickets.map((ticket) => (
              <Link key={ticket.id} to={`/account/tickets/${ticket.id}`} className="block h-full">
                <Card className="h-full transition-colors hover:bg-muted/50 flex flex-col">
                  <CardContent className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="text-lg font-semibold line-clamp-2 mb-4 flex-1">
                      {ticket.concertTitle}
                    </h3>
