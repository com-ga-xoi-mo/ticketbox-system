import type { AssignmentApiClient, MobileSession, StaffAssignment } from '../../api/checkin-mobile-api.types';

export type AssignmentState =
  | { readonly status: 'idle' }
  | { readonly status: 'loading' }
  | { readonly status: 'loaded'; readonly assignments: readonly StaffAssignment[]; readonly selected: StaffAssignment }
...
export class AssignmentController {
  ...
  select(current: AssignmentState, assignmentId: string): AssignmentState {
    ...
    const selected = current.assignments.find((assignment) => assignment.assignmentId === assignmentId);
