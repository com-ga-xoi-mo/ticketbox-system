...
export async function listConcerts(): Promise<Concert[]> {
  return get<Concert[]>(ORGANIZER_CONCERTS_PATH);
}

export async function getConcert(id: string): Promise<Concert> {
  return get<Concert>(`${ORGANIZER_CONCERTS_PATH}/${id}`);
}

export async function createConcert(payload: unknown): Promise<Concert> {
  return post<Concert>(ORGANIZER_CONCERTS_PATH, payload);
}
...
export async function updateConcert(
  ...
  return patch<Concert>(`${ORGANIZER_CONCERTS_PATH}/${id}`, payload);
}

export async function publishConcert(id: string): Promise<Concert> {
  return post<Concert>(`${ORGANIZER_CONCERTS_PATH}/${id}/publish`, {});
}

export async function cancelConcert(id: string): Promise<Concert> {
  return post<Concert>(`${ORGANIZER_CONCERTS_PATH}/${id}/cancel`, {});
}
