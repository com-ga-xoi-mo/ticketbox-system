...
import { useConcerts, usePublishConcertMutation, useCancelConcertMutation } from './hooks';
...
export function ConcertsPage() {
  ...
  const cancelMutation = useCancelConcertMutation();

  const handlePublishConcert = () => {
    if (selectedConcert) {
      publishMutation.mutate(selectedConcert.id);
    }
  };

  const handleCancelConcert = () => {
    if (selectedConcert) {
      cancelMutation.mutate(selectedConcert.id);
    }
  };
    ...
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center" role="status">
        <div
          ...
          aria-hidden="true"
        />
        <p className="mt-4 font-mono text-sm text-on-surface-variant">Đang tải buổi biểu diễn…</p>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col gap-2 overflow-hidden p-4 md:px-8 md:py-3">
      ...
      <div className="flex shrink-0 items-start justify-between">
        <div>
          <h2 className="font-display text-2xl font-bold tracking-tight text-on-surface">
            Quản lý buổi biểu diễn
          </h2>
          <p className="mt-1 text-sm text-on-surface-variant">
            Xem xét, chỉnh sửa, và điều hành tất cả buổi biểu diễn và trạng thái vòng đời của chúng.
          </p>
        </div>
      </div>
      ...
      {concerts.length === 0 ? (
        <div className="flex-1 overflow-y-auto mt-4">
          <ConcertEmptyState canCreate={false} />
        </div>
      ) : (
              ...
              <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/5 bg-surface-variant/20 px-3 py-2">
                ...
                <div className="group relative min-w-[200px] flex-1 max-w-md">
                  ...
                  <input
                    ...
                    aria-label="Tìm kiếm buổi biểu diễn"
                    placeholder="Tìm theo tiêu đề hoặc nghệ sĩ..."
