...
describe('admin concerts api functions', () => {
  ...
  it('listConcerts calls get on the correct path', async () => {
    ...
    await listConcerts();
    expect(client.get).toHaveBeenCalledWith(ADMIN_CONCERTS_PATH);
  });

  it('getConcert calls get with ID', async () => {
    vi.mocked(client.get).mockResolvedValue({});
    await getConcert('123');
    expect(client.get).toHaveBeenCalledWith(`${ADMIN_CONCERTS_PATH}/123`);
  });
  ...
  it('createConcert calls post with payload', async () => {
    ...
    expect(client.post).toHaveBeenCalledWith(ADMIN_CONCERTS_PATH, payload);
  });
  ...
  it('updateConcert calls patch with ID and payload', async () => {
    ...
    expect(client.patch).toHaveBeenCalledWith(`${ADMIN_CONCERTS_PATH}/123`, payload);
  });

  it('publishConcert calls post publish endpoint', async () => {
    vi.mocked(client.post).mockResolvedValue({});
    await publishConcert('123');
    expect(client.post).toHaveBeenCalledWith(`${ADMIN_CONCERTS_PATH}/123/publish`, {});
  });
